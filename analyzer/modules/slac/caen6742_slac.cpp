/**
 * @file   analyzer/modules/slac/caen6742_slac.cpp
 * @author  Wes Gohn <gohn@pa.uky.edu>
 * @date    Fri Jul 18 18:11:35 2014
 * @date    Last-Updated: Sat Jul 19 18:47:11 2014 (-0500)
 *          By : Data Acquisition
 *          Update #: 63
 * @version $Id$
 * 
 ...
 * 
 * @brief   Decoder of CAEN6742 data read out via VME
 * 
 * @details This modules decodes the raw CAEN6742 data read out 
 *          via VME. 
 * 
 * @section Changelog
 * @verbatim
...
/**
   @page page_caen6742_slac caen6742_slac
   @ingroup group_analyzer_modules

   - <b>file</b> : analyzer/modules/slac/caen6742_slac.cpp
   - <b>Input</b> : bank [SF##]
   - <b>Output</b> : \ref caen6742_waveforms

   This analyzer module decodes the ADC memory data of CAEN6742 
   read out via VME interface. 
   
   The decoded waveforms are stored as std::vectors in the array
   \ref caen6742_waveforms as \ref CAEN6742_WAVEFORM structures.
 */
...
static TGraph* gr_bk_size[CAEN6742_NUMBER_OF_BOARDS+1];
...
ANA_MODULE caen6742_slac_module = {
  "caen6742_slac",                   /* module name           */
  "Wes Gohn",                  /* author                */
...
static TTree *t;                  
const int vme_trace_length = CAEN_6742_LN;               
struct s_caen{                                                                                     
  unsigned long long int timestamp;                                                       
  UShort_t trace[CAEN_6742_CH][vme_trace_length];                                            
  unsigned short is_bad_event;
};
                                                                   
struct s_caen br_caen;

static TH2D *h2_wf[CAEN6742_NUMBER_OF_BOARDS][CAEN_6742_CH];  
...
INT module_init(void)
{
  ...
  TFolder *folder = (TFolder*) caen6742_slac_module.histo_folder;

  printf("%s : module_init\n",caen6742_slac_module.name);
  
  // Bank size vs. event number
  for (int iboard=1; iboard<=CAEN6742_NUMBER_OF_BOARDS; iboard++)
    {
  ...
  t->Branch("caen",&br_caen,br_str);

  for(int iboard=0;iboard<CAEN6742_NUMBER_OF_BOARDS; iboard++)
    for(int ich=0; ich<CAEN_6742_CH; ich++){
      h2_wf[iboard][ich] = new TH2D(Form("h2_wf_board_%02d_channel_%d",iboard+1,ich+1),Form("board %02d channel %d",iboard+1,ich+1),512,-0.5,1023.5,10000,-0.5,49999.5);
  }
...
INT module_event(EVENT_HEADER * pheader, void *pevent)
{
   ...
   // clear old data
   /*for (unsigned int iboard=1; iboard<=CAEN6742_NUMBER_OF_BOARDS; iboard++)	 
       for (unsigned int ichannel=1; ichannel<=CAEN_6742_CH; ichannel++)
	 {	     
   ...
   */
   ...
   // Loop over all CAEN6742 MODULES in the experiment
   for (unsigned int i_board=1; i_board<=CAEN6742_NUMBER_OF_BOARDS; i_board++)
     {
       char bank_name[8];
       sprintf(bank_name,"CD%02d",i_board-1);
       ...
       while ( iw < bank_len )
	 {
	   ...
	   for(unsigned int k=0;k<CAEN_6742_CH;k++){
	     //unsigned int j = 0;
	     //WORD *adc = pdata+iw;
	     int adc = pdata[iw++];
	     ...
	     for(int j=0;j<vme_trace_length;j++)
	       {
		 //br_caen.trace[k][j] = (UShort_t)pdata[iw++];
		 br_caen.trace[k][j] = (UShort_t)adc;
		 h2_wf[i_board-1][k]->Fill(j+1,adc);
	       }	   
	     
	     
	     br_caen.timestamp = 0;
	     br_caen.is_bad_event = 0;
	     t->Fill();
	   }
