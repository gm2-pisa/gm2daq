...
namespace amc13{
  ...
  Flash::Flash( AMC13Simple * p_amc13)
  {
  ...
  void Flash::parseMcsFile( std::string p_name ) {
    file_name = p_name ;
    std::string name = boost::to_upper_copy( p_name ) ;
  ...
  std::string Flash::selectMcsFile( int chipNo, std::string ver, std::string chipType) {
	 ...
	 parseMcsFile( dnam ) ; //run parse on file
	 if ( chip_no == chipNo &&  chip_type == chipType) { //version not checked
	   // check version is equal if ver is GOLDEN or HEADER //not yet implemented
	   if (ver == "GOLDEN" || ver=="HEADER") { //check version matches
	     if (ver == version)
	       validFiles.push_back( dnam ) ;
	   }
	   else
	     validFiles.push_back( dnam ) ;
	 }
