/********************************************************************\
   ...
   $Id: mevb.c 5204 2011-10-06 21:03:34Z amaudruz $
...
char mevb_svn_revision[] = "$Id: mevb.c 5204 2011-10-06 21:03:34Z amaudruz $";
...
DWORD start_assemble_time = 0, end_assemble_time = 0, send_event_time = 0;
DWORD dt_receive_fragments, dt_send_fragments;
...
INT load_fragment(void)
{
   ...
   //MIDAS equipment params (those relevent to EB)
   INT type;
   BOOL sendToEventBuilder;
   BOOL enabled;

   /* Get equipment pointer, only one eqp for now */
   eq_info = &equipment[0].info;

   /* Get top-level "Equipment" dir */
   ...
   for (i = 0, nfragment = 0;; i++) {
      ...
      type = 0;
      sendToEventBuilder = FALSE;
      enabled = FALSE;

      //Get equipment name and set as subdirectory name
      ...
      if (key.type == TID_KEY) {
         ...
         size = sizeof(enabled);
         db_get_value(hDB, hSubkey, "common/Enabled", &enabled, &size, TID_BOOL, FALSE);
         size = sizeof(sendToEventBuilder);
         INT success = db_get_value(hDB, hSubkey, "Settings/Globals/Send to Event Builder", &sendToEventBuilder, &size, TID_BOOL, FALSE);

         //Warn user if an EB_EQ type FE does not have "Send to Event Builder" (as this key does not exist by default in MIDAS)
         if( (type & EQ_EB) && (success != DB_SUCCESS) ) {
           cm_msg(MERROR, "load_fragment", "No \"Send to Event Builder\" key found for [%s], defaulting to FALSE",key.name);
         }

         //Print info for equipments
         if (debug) {
           INT eqeb = ( (type & EQ_EB) != 0 ); //1 if equip is type EQ_EB, 0 otherwise
	   printf("Equipment name : %s, Type: 0x%x, EQ_EB: %i, Send to EB: %i, Buffer: %s, Format: %s, Enabled: %i\n", 
             key.name, eqeb, type, sendToEventBuilder, buffer, format, enabled);
         }//end if debug

         /* Check if equipment matches EB requirements 
          *   1) equipment has type EQ_EB
          *   2) buffer name matches EB expected value (as passed to mevb using '-b' arg at 
          *      run time, we use '-b BUF', so equipment bufferss must be 'BUF1,' BUF2' etc)
          *   3) formats of EB and FE must match (must both be 'MIDAS')
          *   4) equipment is enabled
          *   5) equipment has "Send to Event Builder" set to TRUE
          */
	 if (   (type & EQ_EB)
             && (strncmp(buffer, buffer_name, strlen(buffer_name)) == 0) 
             && (strncmp(format, eq_info->format, strlen(format)) == 0)
             && (enabled == TRUE)
             && (sendToEventBuilder == TRUE) )
         {
      ...
      }//end if (key.type == TID_KEY)

   }//end for (i = 0, nfragment = 0;; i++)


   if (nfragment > 0)
      printf("Found %d fragment(s) for event building\n", nfragment);
   else
      //printf("Found no fragments for event building\n");
      cm_msg(MERROR, "load_fragment", "Found no fragments for event building");
...
INT scan_fragment(void)
{
   ...
   do {
	 ...
	 if (debug) printf("stats: events sent, bytes_sent, time interval %d %d %f \n",
			   eq->events_sent,eq->bytes_sent,((actual_millitime - last_time) / 1000.0) );
         eq->bytes_sent = 0;
         eq->events_sent = 0;
...
INT handFlush()
{
   ...
   for (i = 0; i < nfragment; i++) {
      do { 
         ...
         if (ebset.preqfrag[i]) {
            size = max_event_size;
	        //printf("before bm_receive_event call, size = %i \n");
...
INT close_buffers(void)
{
   ...
   printf("calling source_unbooking...\n");
...
INT source_scan(INT fmt, EQUIPMENT_INFO * eq_info)
{
   ...
   start_assemble_time = ss_millitime();
   ...
   if (i == nfragment) {
     ...
     end_assemble_time = ss_millitime();
	...
	final header information */
     ...
     if (!ebset.user_build) {
       for (i = 0; i < nfragment; i++) {
	 if (ebset.preqfrag[i]) {
	   if (debug) printf("mevb assemble fragment %d\n",i);
     ...
     if (debug) printf("events sent %d\n",equipment[0].events_sent);
     
     /* timing information */
      send_event_time = ss_millitime();
      dt_receive_fragments = end_assemble_time - start_assemble_time;
      dt_send_fragments = send_event_time - end_assemble_time;
      printf("receive dt, send dt, tstart, tassemble, tsend %d %d %d %d %d\n", 
	     dt_receive_fragments, dt_send_fragments, start_assemble_time, end_assemble_time, send_event_time );
