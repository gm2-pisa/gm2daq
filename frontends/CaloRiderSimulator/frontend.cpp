 ...
 * @author  Wes Gohn/Tim Gorringe
 * @date    Thu May 24 10:08:59 2012 (-0500)
 * @date    Last-Updated: Thu Oct  1 13:46:21 2015 (-0500)
 *          Update #: 627
 * @version $Id$
 *
 * @copyright (c) (g-2) collaboration 
 *
 * @brief   Rider trigger frontend for (g-2)
 *
 * @ingroup page_frontends
 * 
 * @details This frontend triggers the rider to send data
 *          
 *
 * 
 * @section Changelog
...
/*declare AMC13 util and AMC13 library access objects   */
  ...
  //global trigger counter for riders
  uint32_t rider_trigger_counter;
  uint32_t memloc1;
  /** The frontend name (client name) as seen by other MIDAS clients   */
  char *frontend_name = "RiderTrigger";
...
EQUIPMENT equipment[] = { 
  {
   "RiderTrigger%02d",                  /* equipment name */
...
INT frontend_init()
{
  ...
  if ( ridertrigger_ODB_init() != SUCCESS )
    {
      return FE_ERR_ODB;
    }
  dbprintf("%s(%d): ridertrigger_ODB_init() done \n", __func__, __LINE__);                                                    
  ...
  */
  ...
  //initialize AMC13
  uhal::disableLogging();//disable uHal logging

  //uhal::ConnectionManager connectionManager("file://RiderConnection.xml");
  uhal::ConnectionManager connectionManager("file://connection.xml");
  dbprintf("%s(%d): Rider Connection xml \n", __func__, __LINE__);                                   
  wfd=new uhal::HwInterface(connectionManager.getDevice("wfd"));
  dbprintf("%s(%d): Rider .getDevice \n", __func__, __LINE__);                                   
  //wfd[1]=new uhal::HwInterface(connectionManager.getDevice("wfd1_sn2"));
  //reset WFD1 & 2
  //wfd->getNode("ctrl.rst").write(1);
  //wfd->dispatch();
  //wfd[1]->getNode("ctrl.rst").write(1);
  //wfd[1]->dispatch();
  ...
  rider_trigger_counter=0;
  memloc1 = 0;

  printf("RIDER INITIALIZED\n");
...
INT poll_event(INT source, INT count, BOOL test)
{
  ...
  INT retval = 0; 
  //printf("poll_event, source %i, count %i, test %i\n",source,count,test);
...
INT rpc_g2_end_of_fill(INT index, void *prpc_param[])
{
  ...
  */
  ...
  //write to WFD (this was for fake data with 1ch rider)
  //std::cout << "Sending WR_REG commands to ADC memory" << std::endl;
  //riderlib->WR_REG(wfd, 0x0000000d, memloc1);
  //memloc1++;
  //iderlib->RD_REG(wfd, 0x0000000d);
  //riderlib->WR_REG(wfd, 0x0000000d, 0x00000000);
  //riderlib->WR_REG(wfd, 0x0000000e, 0xaaaaaaaa);
  //riderlib->WR_REG(wfd, 0x0000000e, rider_trigger_counter);
  //riderlib->WR_REG(wfd, 0x0000000d, 0x00000001);
  //riderlib->WR_REG(wfd, 0x0000000e, 0xbbbbbbbb);
  //riderlib->WR_REG(wfd, 0x0000000d, 0x00000002);
  //riderlib->WR_REG(wfd, 0x0000000e, 0xcccccccc);
  //riderlib->WR_REG(wfd, 0x0000000d, 0x00000003);
  //riderlib->WR_REG(wfd, 0x0000000e, 0xdddddddd);
  //riderlib->WR_REG(wfd, 0x0000000d, 0x00000004);
  //riderlib->WR_REG(wfd, 0x0000000e, 0xeeeeeeee);
  //riderlib->WR_REG(wfd, 0x0000000d, 0x00000005);
  //riderlib->WR_REG(wfd, 0x0000000e, 0xffffffff);

  //std::cout << "Sending WR_REG commands to ADC header FIFO" << std::endl;
  /*
  riderlib->WR_REG(wfd, 0x0000000f, 0x00000000);
  riderlib->WR_REG(wfd, 0x0000000f, rider_trigger_counter);
  riderlib->WR_REG(wfd, 0x0000000f, 0x00000006);
  riderlib->WR_REG(wfd, 0x0000000f, 0x00000000);
  riderlib->WR_REG(wfd, 0x0000000f, 0x00000006);
  riderlib->WR_REG(wfd, 0x0000000f, 0x00000000);
  rider_trigger_counter++;
  std::cout<<"Rider Trigger Counter increment = "<<std::hex<<rider_trigger_counter<<std::endl;
  std::cout << "Sending trigger to AMC13" << std::endl;
  */

  //amc13lib->AMC13_Write(amc13);
  // amc13->dispatch();

  //not needed as of 7/8/2015, Rider gets TTC trigger from AMC13
  //wfd->getNode("wo.trigger").write(1);
  //wfd->dispatch();
  
  //std::cout << "Sending done signals for the 5 channels" << std::endl;

  std::cout << "Sending trigger to trigger manager" << std::endl;
  ...
  std::cout << "Resetting done signals for the 5 channels" << std::endl;
